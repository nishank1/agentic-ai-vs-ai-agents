...
def test_agent_tools_behave_as_expected() -> None:
    ...
    assert (
        tools_module.count_words.invoke({"text": "agentic systems need controls"}) == 4
    )

    timestamp = tools_module.get_current_utc_time.invoke({})
    assert timestamp.endswith("+00:00")


def test_workflow_graphs_compile() -> None:
    workflow_module = load_example_module(
        "workflow_03", "03_agentic_workflow/workflow.py"
    )
    multi_agent_module = load_example_module(
        "workflow_04", "04_multi_agent/workflow.py"
    )
    demo_module = load_example_module(
        "workflow_05", "05_real_world_demo/research_agent.py"
    )
